...
Goals for this fork include:
- More usable out-of-the-box HTML outputs:
    - Sortable tables
    - Table headers to scroll with the page for ease of reference
    - Tooltips explaining abbreviations
    - Hide/show columns?
- Additional C/C++ and Java metrics:
    - Average number of lines of code per method within each module
    - Average nesting within each module
    - Largest number of lines of code per method within each module
    - Largest nesting within each module
    - Average Product of LoC/Method * Nesting
    - Largest Product of LoC/Method * Nesting

Seeing as how I'm the sole maintainer of this fork, some changes that
may occur:

- I do not share Tim's appreciation of multiple compiler support. I will
  be using GCC for this project. I may auto-conf-ify the project,
  removing support for other compilers; if so, I shall commit to
  maintaining MinGW builds for the Windows environment. I am likely to check
  in Debian packaging files.
- I will not be doing anything with the Visual Studio plugin, so, mind the
  bitrot.
- I don't like the whitespace style, and am using the more conventional
  K&R style with 4 spaces per tab. I'm not inclined to bother changing the
  existing code unless I trip over it, though.
